...
build_expected_output_manifest_from_config <- function(cfg, out_dir, output_level = "standard") {
  ...
  duplicate_only_export <- full_debug_output_enabled
  ...
  if ((standard_output_enabled || full_debug_output_enabled) && comparison_mode %in% c("multigroup", "both")) {
    optional_files <- c(
      optional_files,
      file.path(out_dir, "<MODEL>", "exports", "stats", "MULTIGROUP_README.txt"),
  ...
  if (full_debug_output_enabled) {
    optional_files <- c(
      optional_files,
      file.path(out_dir, "<MODEL>", "exports", "stats", "significant", "*.txt")
    )
  }
