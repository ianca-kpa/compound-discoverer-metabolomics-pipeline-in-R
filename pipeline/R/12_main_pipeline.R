...
run_untargeted_pipeline <- function() {
  ...
  with_console_capture_to_file(log_path, {
    ...
    # -------------------------------------------------------------------------
    # STEP 08 — Applying main normalization
    # -------------------------------------------------------------------------
    normalization_mode_local <- toupper(as.character(get0("normalization_mode", ifnotfound = "PQN", inherits = TRUE)))
    if (normalization_mode_local == "LOESS") normalization_mode_local <- "QC_LOESS"
    if (normalization_mode_local == "NONE") normalization_mode_local <- "none"
    t_step <- step_start(8, 15, paste0("Applying ", toupper(normalization_mode_local), " normalization"))

    normalization_bundle <- NULL

    if (normalization_mode_local == "none") {
      assay_num_pqn <- assay_num_weight
      normalization_bundle <- list(assay_num_none = assay_num_pqn)
      step_info("No main normalization applied. Continuing with the post-weight matrix.")
      ...
      assay_num_pqn <- normalization_bundle$assay_num_loess

      if (!minimal_output) {
        out_loess <- file.path(paths$global$exports, "06_loess_qc_correction_audit_weight_then_LOESS.csv")
        write_csv_safe(normalization_bundle$loess_tbl, out_loess)
        log_written_object(log_path, out_loess, normalization_bundle$loess_tbl, note = "QC-LOESS correction audit")
      }

      step_info(
        "QC-LOESS normalization applied. Corrected features: ",
        sum(normalization_bundle$loess_tbl$corrected, na.rm = TRUE),
        " / ",
        nrow(normalization_bundle$loess_tbl)
      )
    } else if (normalization_mode_local == "PQN") {
      normalization_bundle <- normalize_pqn_qc_ref(assay_num_weight, qc_idx)
      assay_num_pqn <- normalization_bundle$assay_num_pqn

      if (!minimal_output) {
        out_pqn <- file.path(paths$global$exports, "06_pqn_factors_weight_then_PQN.csv")
        df_pqn <- bind_cols(metadata_aligned %>% select(sample, type), normalization_bundle$pqn_tbl)
      ...
      stop("Unsupported normalization_mode: ", normalization_mode_local)
    }

    step_ok(paste0("Applying ", toupper(normalization_mode_local), " normalization"), t_step)
    ...
    suppressWarnings({
      ...
      if (isTRUE(make_heatmap_by_model)) {
        step_info("TOP heatmaps per model (ranking from official stats tables)...")
      ...
      if (isTRUE(make_heatmap_by_model_sex)) {
        step_info("TOP heatmaps per model split by sex (ranking from official stats tables)...")
