...
normalize_sample_type <- function(type, sample, group = NA_character_) {
  sample <- stringr::str_trim(as.character(sample))
  s <- tolower(stringr::str_trim(as.character(type)))
  g <- tolower(stringr::str_trim(as.character(group)))
  s[s %in% c("", "na", "n/a", "nan", "unknown", "unk", "-", "null")] <- NA_character_
  g[g %in% c("", "na", "n/a", "nan", "unknown", "unk", "-", "null")] <- NA_character_

  qc_values <- c("qc", "pooled qc", "pool qc", "pooled_qc", "pool_qc", "quality control", "quality_control")

  dplyr::case_when(
    stringr::str_detect(sample, stringr::regex("^QC", ignore_case = TRUE)) ~ "QC",
    s %in% qc_values ~ "QC",
    g %in% qc_values ~ "QC",
    s %in% c("sample", "biological sample", "biological_sample") ~ "Sample",
    is.na(s) ~ "Sample",
...
clean_metadata <- function(metadata) {
  ...
  alias_map <- list(
    ...
    batch     = c("batch", "batch_id", "analysis_batch", "run", "run_batch")
  )
  ...
  metadata %>%
    dplyr::mutate(
      ...
      type = if (!("type" %in% names(.))) {
        normalize_sample_type(NA_character_, sample, group)
      } else {
        normalize_sample_type(type, sample, group)
      }
    ) %>%
    { if ("batch" %in% names(.)) { .$batch <- stringr::str_trim(as.character(.$batch)) }; . }
}
